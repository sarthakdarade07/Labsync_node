...
exports.getById = async (req, res) => {
    try {
        const data = await SystemSetting.findById(req.params.id);
...
exports.create = async (req, res) => {
    try {
        const data = await SystemSetting.create(req.body);
...
exports.update = async (req, res) => {
    try {
        const data = await SystemSetting.findByIdAndUpdate(req.params.id, req.body, { new: true });
        if (!data) return res.status(404).json({ success: false, message: 'Not found' });
...
exports.delete = async (req, res) => {
    try {
        const data = await SystemSetting.findByIdAndDelete(req.params.id);
        if (!data) return res.status(404).json({ success: false, message: 'Not found' });
